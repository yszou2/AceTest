...
var render = dojo.hitch({template: template}, function(context, when_null){
if(!when_null){when_null = '-'}
if(!dojo.isArray(context)){
var s = this.template.replace(/%\((.*?)\)s/g, function(m, n){
return context[n] ? safe(context[n]) : when_null;
});
} else {
var s = this.template.replace(/%s/g,
dojo.hitch({n: 0, l: context.length}, function(m){
return (this.n < this.l ? safe(context[this.n++]) : when_null);
}));
}
return s;
});
...
n.o_height = dojo.style(n, 'height');
dojo.toggleClass(n, 'aw_sb_items_on');
dojo.toggleClass(n, 'aw_sb_items_off');
n.show = dojo.hitch({node: n}, this.item_node_show);
n.hide = dojo.hitch({node: n}, this.item_node_hide);

dojo.query('li', n).forEach(function(item){
item.AW_ONCLICK_BIND = dojo.connect(item, 'onclick', {node: n},
function(eventObj){this.node.hide()});
});
...
sn.AW_ONCLICK_BIND = dojo.connect(sn, 'onclick', {sn: sn}, function(eventObj){
dojo.stopEvent(eventObj);
if(this.sn.disabled){return}
...
this.n.layout = dojo.create('div', {style: 'position: absolute; background-color: white; padding: 5px; border: 1px solid #0184B7; font-family: monospace; font-size: 12px; letter-spacing: 1px; -moz-box-shadow: 3px 3px 5px #333; -webkit-box-shadow: 3px 3px 5px #333;'}, dojo.body(), 'last');
...
dojo.xhrGet({
url: URL.case + '?t=' + (new Date()).getTime(),
handleAs: 'json',
defer: this.defer,
filter: this.filter,
an: an,
...
node.get_value = dojo.hitch({n: node}, function(){
var o = {};
...
node.run = dojo.hitch({n: node}, function(){
this.n.is_running = true;
var v = this.n.get_value();
var rc = RunCase(v);
...
if(!url.match(/^http:\/\//)){
if(ph != ''){ url = pub_host + url}
else{url = 'http://localhost:8000' + url}
}
...
this.n.step_position = this.n.y_position[1] - this.n.y_position[0];
